...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
    ...
    <title>Cearchi - Resultado de relatorio de serviço</title>
...
<?php
 ...
 @$data_inicial = $_POST['data_servico'];
  ...
  if(strcmp($regiao, '') != 0)
  {
    ...
    $sql = mysql_query("SELECT * FROM solicitacao_de_servico");
    ...
    while ($linha = mysql_fetch_array($sql)) {
      ...
      $id_cont = $linha['fk_id_user_contratante'];
      $id_ts = $linha['fk_id_ts'];
      ...
      if($data_ss_formatada >= $data_inicial_formatada && $data_ss_formatada <= $data_final_formatada){
        $sql1 = mysql_query("SELECT * FROM usuario INNER JOIN estados ON usuario.estado_user = estados.sigla WHERE id_user like '$id_cont'") or die(mysql_error());
        $row = mysql_num_rows($sql1);
        if($row > 0){
          @$cont++;  
          echo "$cont dsadfhaspuidfh";
        }
      }
    } 
